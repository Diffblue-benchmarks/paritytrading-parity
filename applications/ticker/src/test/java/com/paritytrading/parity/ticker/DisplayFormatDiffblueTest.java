...
class DisplayFormatDiffblueTest {
  ...
  @Test
  @DisplayName(
      "Test update(OrderBook, boolean); given zero; when OrderBook getBidSize(long) return zero; then calls getAskSize(long)")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void DisplayFormat.update(OrderBook, boolean)"})
  void testUpdate_givenZero_whenOrderBookGetBidSizeReturnZero_thenCallsGetAskSize() {
    // Arrange
    Instrument instrument = mock(Instrument.class);
    when(instrument.getPriceFactor()).thenReturn(10.0d);
    when(instrument.getSizeFactor()).thenReturn(10.0d);
