...
class TradeProcessorDiffblueTest {
   ...
   * <ul>
   *   <li>Given two.
   *   <li>When {@link PMR.Version} (default constructor) {@link PMR.Version#version} is two.
   *   <li>Then calls {@link Instruments#getPriceWidth()}.
   * </ul>
   *
  ...
  @Test
  @DisplayName(
      "Test version(Version); given two; when Version (default constructor) version is two; then calls getPriceWidth()")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  ...
  void testVersion_givenTwo_whenVersionVersionIsTwo_thenCallsGetPriceWidth() {
    ...
    when(instruments.getSizeWidth()).thenReturn(1);
    DisplayFormat listener = new DisplayFormat(instruments);
    TradeProcessor tradeProcessor = new TradeProcessor(listener);
